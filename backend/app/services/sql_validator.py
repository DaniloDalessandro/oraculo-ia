...
- Apenas tabelas autorizadas podem ser consultadas
...
# Tabelas autorizadas para consulta via IA
...
# Palavras-chave que jamais devem aparecer em queries geradas
...
def _collect_tables_from_statement(statement: Statement, tables: set[str]) -> None:
    ...
    for token in statement.flatten():
        ...
        # Palavras-chave que indicam início de referência a tabela
        ...
        # Token após FROM/JOIN é o nome da tabela
...
def _inject_limit(sql: str, limit: int) -> str:
    ...
    if "LIMIT" in upper:
        ...
        # Remove ponto-e-vírgula final e adiciona LIMIT
...
def validate_and_prepare(raw_sql: str) -> str:
    ...
    # Bloqueia múltiplas statements (ponto-e-vírgula no meio)
    ...
    # Checa palavras-chave bloqueadas (case-insensitive)
    sql_upper = sql.upper()
    ...
    # Verifica que começa com SELECT
    parsed = sqlparse.parse(sql)
    if not parsed:
